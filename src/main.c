...
static bool
read_program (char * path, uint * program)
{
  char opcode_str[7] = { 0  };
  uint r_a;
  uint r_b;
...
void main (int argc, char *argv)
{
  ...
  read_program ("input/program.qc", program);
  ...
  while (!halted) {
    ...
    switch (OP_CODE(instruction)) {
      case ADDITION:
        debug_msg ("ADDITION");
        read_standard_instruction_registers (instruction, &reg_a, &reg_b, &reg_c);
